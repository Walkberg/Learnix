...
interface UpdateFlashcardCommand {
  flashcardId: string;
  userId: string;
  patch: Partial<Pick<Flashcard, 'question' | 'answer'>>;
}
...
export class UpdateFlashcardUseCase {
  ...
  async execute({ flashcardId, userId, patch }: UpdateFlashcardCommand) {
    ...
    const count = await this.flashcardRepo.updateById(
      flashcardId,
      patch as any,
    );
