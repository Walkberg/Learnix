...
- [ ] T001 [P] Scaffold the frontend using Vite CLI into `frontend/` (commands):
...
- [ ] T002 [P] Scaffold the backend using the NestJS CLI into `backend/` (commands):
	Run in repository root:
	- `npx @nestjs/cli new backend --package-manager npm`
	- `cd backend; npm install`
	Result: `backend/` folder with `package.json`, `tsconfig.json`, `src/main.ts`, `src/app.module.ts`, and feature folders.
- [ ] T003 Create `backend/prisma/schema.prisma` (if missing) and add `backend/.env.example` with `DATABASE_URL` (dev: `file:./dev.db`)  
...
- [ ] T011 [P] Setup Prisma client and migration scripts: add `backend/package.json` scripts `prisma:generate`, `prisma:migrate` and run `npx prisma generate`
- [ ] T012 [P] Implement AI Adapter scaffold: create `backend/src/features/ai/adapter.ts` (interface) and `backend/src/features/ai/openai-adapter.ts` + `backend/src/features/ai/mock-adapter.ts` (for CI/local)
- [ ] T013 Create job queue scaffolding (optional, feature-flagged): `backend/src/jobs/worker.ts`, `backend/src/jobs/index.ts`, and `backend/src/config/queue.ts` (guarded by `USE_QUEUE` env var)
...
- [ ] T100 [US1] Create backend auth domain & model files: `backend/src/features/auth/domain/user.entity.ts`, `backend/src/features/auth/dto/` and `backend/src/features/auth/auth.module.ts`
- [ ] T101 [US1] Implement auth service (registration/login) in `backend/src/features/auth/application/auth.service.ts` and password hashing (bcrypt) in `backend/src/features/auth/infrastructure/password.util.ts`
- [ ] T102 [US1] Add auth controller endpoints in `backend/src/features/auth/auth.controller.ts` (`POST /api/auth/signup`, `POST /api/auth/login`, `POST /api/auth/logout`)
- [ ] T103 [US1] Add auth guards and JWT strategy: `backend/src/common/guards/jwt.guard.ts`, `backend/src/common/strategies/jwt.strategy.ts`
- [ ] T104 [P] Create frontend auth pages & forms: `frontend/src/features/auth/SignUp.tsx`, `frontend/src/features/auth/Login.tsx`, using `react-hook-form` + `zod` schemas in `frontend/src/features/auth/schema.ts`
- [ ] T105 [US1] Create backend acceptance tests stubs (executable): `backend/test/acceptance/auth.spec.ts` (wire to testApp harness) and CI job to run them
